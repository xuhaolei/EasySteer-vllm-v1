...
class LoReFTAlgorithm(AlgorithmTemplate):
    ...
    - Payload is a dict containing rotate_layer, learned_source_weight,
    learned_source_bias
    ...
    def _apply_loreft_transformation(
        self, hidden_states: torch.Tensor, loreft_params: dict
    ) -> torch.Tensor:
        """
        ...
        Args:
            ...
            loreft_params: Dictionary containing rotate_layer, learned_source parameters
            and scale_factor
    ...
    def load_from_path(cls, path: str, device: str, **kwargs) -> dict:
        ...
        if config is None:
            raise ValueError(
                "LoReFTAlgorithm.load_from_path requires 'config' in kwargs"
            )
        ...
        if not os.path.isdir(path):
            raise ValueError(
                f"For LoReFT algorithm, path must be a directory. Got: {path}"
            )
        ...
        if config_layer_idx is None:
            raise ValueError(
                ...
                f"filename {bin_filename}"
            )

        if target_layers and config_layer_idx not in target_layers:
            raise ValueError(
                f"Layer mismatch: config specifies layer {config_layer_idx}, but "
        ...
        adapter_dtype = (
            config.adapter_dtype if hasattr(config, "adapter_dtype") else torch.float16
        )
