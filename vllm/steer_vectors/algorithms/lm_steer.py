...
class LMSteerAlgorithm(AlgorithmTemplate):
    ...
    @classmethod
    def load_from_path(
        cls, file_path: str, device: str, config=None, target_layers=None
    ):
        """Load LM-Steer parameters from pt file."""

        try:
            # Load pt file, set weights_only=False to allow loading argparse.Namespace
            # and other objects
            ...
            if isinstance(state_dict, list) and len(state_dict) > 1:
                # logger.info(f"Detected list structure, trying to extract parameters
                # from element[1]")
                params_dict = state_dict[1]

                if (
                    isinstance(params_dict, dict)
                    and "projector1" in params_dict
                    and "projector2" in params_dict
                ):
                    # This is the low-rank optimization form
                    # logger.info(f"Found low-rank optimization projector1 and
                    # projector2 parameters")
            ...
            if projector1 is None or projector2 is None:
                logger.error("Could not find projector matrices in file %s", file_path)
                raise ValueError("Projector matrices not found in pt file")

            # Get data type from config, with default value
            adapter_dtype = (
                config.adapter_dtype
                if hasattr(config, "adapter_dtype")
                else torch.float16
            )
            ...
            # logger.info(f"Loaded low-rank projection matrices P1:
            # {projector1_tensor.shape}, P2: {projector2_tensor.shape}")

            return {"layer_payloads": layer_payloads}

        except Exception as e:
            logger.error("Failed to load LM-Steer parameters from %s: %s", file_path, e)
            raise RuntimeError("Failed to load LM-Steer parameters") from e
