...
class LinearTransformAlgorithm(AlgorithmTemplate):
    ...
    def load_from_path(
        ...
        try:
            ...
            # Check if it's a LinearTransport object, access attributes directly instead
            # of using get method
            if hasattr(data, "A_") and hasattr(data, "B_"):
                ...
                try:
                    ...
                    logger.error(
                        "Cannot extract A_ and B_ from data type: %s",
                        type(data),
                    )
                    raise ValueError(
                        "Unsupported data format. Neither a dict nor has A_/B_ "
                        f"attributes: {type(data)}"
                    )

            if weight is None:
                logger.error(
                    "Failed to find weight (A_) in data of type %s",
                    type(data),
                )
                raise ValueError("Weight matrix (A_) not found in pkl file")
            ...
            bias_tensor = (
                torch.tensor(bias, device=device, dtype=adapter_dtype)
                if bias is not None
                else None
            )
            ...
            logger.error(
                "Failed to load linear transform parameters from %s: %s",
                file_path,
                e,
            )
            raise RuntimeError("Failed to load linear transform parameters") from e
